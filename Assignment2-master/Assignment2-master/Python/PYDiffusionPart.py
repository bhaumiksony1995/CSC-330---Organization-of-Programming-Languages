...
cube=[[[0.0 for k in range(0,maxsize)] for j in range(0,maxsize)] for i in range(0,maxsize)]

partsize = maxsize / 2

part=input("Do you want to add a partition? (1 = Yes / 2 = No) ")
        ...
        for j in range(0,maxsize):
            for k in range(0,maxsize):
                if (cube[i][j][k] >= 0):
                    ...
                    if ((i != maxsize-1) and (cube[i+1][j][k] >= 0)):
                        change = (cube[i][j][k] - cube[i+1][j][k]) * DTerm
                        cube[i][j][k]=cube[i][j][k] - change
                        cube[i+1][j][k]=cube[i+1][j][k] + change
                    ...
                    if ((j != maxsize-1) and (cube[i][j+1][k] >= 0)):
                        change = (cube[i][j][k] - cube[i][j+1][k]) * DTerm
                        cube[i][j][k]=cube[i][j][k] - change
                        cube[i][j+1][k]=cube[i][j+1][k] + change
                    ...
                    if ((k != maxsize-1) and (cube[i][j][k+1] >= 0)):
                        change = (cube[i][j][k] - cube[i][j][k+1]) * DTerm
                        cube[i][j][k]=cube[i][j][k] - change
                        cube[i][j][k+1]=cube[i][j][k+1] + change
    time = time + timestep
